...
![[Pasted image 20251107090753.png]]
...
onsider a company with offices in different geographical locations. Can this company connect all its offices and sites to the main branch so that any device can access the shared resources as if physically located in the main branch? The answer is yes; furthermore, the most economical solution would be setting up a virtual private network (VPN) using the Internet infrastructure. The focus here is on the V for Virtual in VPN.
...
![[Pasted image 20251107093205.png]]


Nice — here’s a single compact, hands-on recap of **everything we covered** so you can copy/paste and act immediately. I’ll group it into **concepts**, **exact commands/filters**, **Wireshark steps**, **troubleshooting**, and **practice checklist**.

# 1) Short concepts recap

- SMTP = send mail. POP3 / IMAP = retrieve mail.
    
- `S` at end (SMTPS, POP3S, IMAPS) = protocol over TLS (encrypted). Common implicit-TLS ports: SMTPS 465, IMAPS 993, POP3S 995. STARTTLS/STLS = in-band upgrade on plaintext ports (SMTP 25/587, IMAP 143, POP3 110).
    
- `ssl-key.log` produced by Chromium (`--ssl-key-log-file=~/ssl-key.log`) contains TLS secrets that Wireshark can use to decrypt TLS/HTTP traffic from that browser.
    
- If TLS is decrypted, you can inspect HTTP (including POST bodies with credentials). If traffic is HTTP/2 or QUIC, slightly different inspection technique is needed. If TLS isn’t decrypted, you’ll only see TLS packets, not credentials.
    

# 2) Exact commands to run on the TryHackMe box (copy/paste)

Check key log exists & has contents:

`wc -l ~/ssl-key.log tail -n 5 ~/ssl-key.log # lines should look like: CLIENT_RANDOM <hex> <hex...>`

Open TLS sessions from the terminal (useful for labs):

`# implicit TLS (connect and inspect) openssl s_client -connect target:465 -crlf        # SMTPS openssl s_client -connect target:993 -crlf        # IMAPS openssl s_client -connect target:995 -crlf        # POP3S  # STARTTLS upgrade (inspect plaintext then STARTTLS) openssl s_client -starttls smtp -connect target:587 -crlf openssl s_client -starttls imap -connect target:143 -crlf openssl s_client -starttls pop3 -connect target:110 -crlf`

Capture traffic with tcpdump (save full packets):

`# capture everything to a file (-s 0 ensures full packet) sudo tcpdump -i tun0 host <target-ip> -s 0 -w ~/Documents/capture.pcapng  # read pcap tcpdump -r ~/Documents/capture.pcapng -nn | head`

Base64 decode an Authorization header:

`echo 'BASE64STRING' | base64 -d # or Python python3 -c "import base64;print(base64.b64decode('BASE64STRING'))"`

# 3) Wireshark — exact steps to decrypt & find credentials

1. Ensure `ssl-key.log` is nonempty and note the full path (e.g. `/home/youruser/ssl-key.log`).
    
2. In Wireshark: **Edit → Preferences → Protocols → TLS (or Transport Layer Security)** → set **(Pre)-Master-Secret log filename** to that path → restart Wireshark or re-open the capture.
    
3. Verify decryption: open any TLS packet and expand it. If you see **Application Data → Hypertext Transfer Protocol** (or `HTTP/2` frames), decryption worked.
    

Useful display filters to find credentials (copy/paste into the display filter bar):

HTTP (common login POSTs):

`http.request.method == "POST"`

Broader HTTP families:

`http || http2`

Search for keywords (works across all protocols if decrypted or plaintext):

`frame contains "password" || frame contains "passwd" || frame contains "username" || frame contains "login" || frame contains "email"`

JSON logins:

`http.request.method == "POST" && http.content_type contains "json" frame contains "{\"username\"" || frame contains "\"password\""`

Authorization / cookies / tokens:

`http.authorization http.cookie frame contains "Authorization:" || frame contains "Bearer " || frame contains "access_token"`

QUIC / HTTP/3 (UDP + 443):

`quic || udp.port == 443`

How to extract creds once you find a candidate packet:

- Right-click → **Follow → HTTP Stream** (or TCP Stream / QUIC Stream). The full request + response will open.
    
- Look in the request body for `username=...&password=...` or JSON `{"username":"...","password":"..."}`.
    
- If you see `Authorization: Basic <base64>` copy the base64 and decode with the commands above.
    

# 4) If `http.request.method == "POST"` returned nothing — why, and what to try

Common causes:

- TLS not actually decrypted (ssl-key.log empty or wrong path). Check `tail -n 5 ~/ssl-key.log`.
    
- Login uses **HTTP/2** or **QUIC (HTTP/3)** — `http2` and `quic` filters needed.
    
- Login flow uses JavaScript to POST to an API on another domain (check all hosts your browser contacted).
    
- Auth uses OAuth / redirects / token exchange — you may not see raw username/password; you may see tokens instead.
    
- Client-side encryption / hashing before send — the cleartext password never leaves the browser.
    
- Login via IMAP/POP/SMTP or other protocol (rare for web login).
    

What to run now (exact filters to try):

`http2 quic || udp.port == 443 frame contains "application/x-www-form-urlencoded" frame contains "application/json" frame contains "Authorization" || frame contains "Bearer " || frame contains "access_token" ip.addr == <your_machine_ip>   # then inspect traffic to other hosts`

# 5) Quick troubleshooting checklist (do these now)

1. `tail -n 5 ~/ssl-key.log` → must show `CLIENT_RANDOM` lines.
    
2. Open a TLS packet in Wireshark → do you see HTTP under Application Data? If no → key mismatch.
    
3. Run `http2` and `quic` filters as above.
    
4. Do a broad keyword search: `frame contains "password"` (search Packet Bytes with Ctrl+F if display filter returns nothing).
    
5. Inspect DNS / SNI to see which host got contacted (`frame contains "Host:"` or check TLS Server Name Indication in the TLS handshake). Then filter `ip.addr == <that-server-ip>` and inspect those streams.
    

# 6) Practical practice tasks (do these in order)

- Reproduce a browser login while tcpdump is running. Then open the pcap and use `http.request.method == "POST"` and `frame contains "password"`.
    
- Force a site to use HTTP/2 and practice examining HTTP/2 frames (`http2` filter).
    
- Capture a QUIC session and try `quic` + check if Wireshark decrypts (it can be tricky).
    
- Use `openssl s_client` to connect to SMTPS/IMAPS/POP3S and manually issue `USER`/`PASS` (only on lab targets).
    

# 7) Final tips & forensic sanity checks

- If TLS is decrypted, credentials will be visible in packet bytes — copy printable bytes if needed.
    
- Look for POSTs to third-party domains (the actual auth endpoint might be a different server).
    
- Use **Follow Stream** to reconstruct entire conversation — often easier than hunting single packets.
    
- Be careful: only test on TryHackMe/authorized targets.
---
